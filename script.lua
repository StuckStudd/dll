...
-- Target Key Link & Valid Keys
local KEY_LINK = "https://work.ink/219m/twks-hub"
local VALID_KEYS = {
	["TWKS-2026-ACCESS"] = true,
	["TWKS-FREE-KEY"] = true,
	["TWKS-VIP-999"] = true
}

-- =======================================================
-- CUSTOM KEY SYSTEM UI
-- =======================================================

local KeyScreen = Instance.new("ScreenGui")
KeyScreen.Name = "TWKS_KeySystemUI"
...
Title.Text = "TWKS Hub - Key System"
...
Subtitle.Text = "Get your access key via the link below"
...
CopyBtn.Size = UDim2.new(0.4, 0, 0, 38)
CopyBtn.Position = UDim2.new(0.075, 0, 0.65, 0)
CopyBtn.BackgroundColor3 = Color3.fromRGB(45, 90, 185)
...
SubmitBtn.Position = UDim2.new(0.525, 0, 0.65, 0)
SubmitBtn.BackgroundColor3 = Color3.fromRGB(40, 160, 90)
...
local function initMainScript()
	...
	-- =======================================================
	-- SEARCH FUNCTIONS (FIXED DOUBLE DOORS)
	-- =======================================================
	...
	-- Исправленный поиск для Double Doors
	local function getTargetDoubleDoors()
		local targets = {}
		pcall(function()
			local searchRoots = {Workspace}
			if getgenv and getgenv().Ugc then table.insert(searchRoots, getgenv().Ugc) end

			for _, root in ipairs(searchRoots) do
				for _, obj in ipairs(root:GetDescendants()) do
					local nameLower = obj.Name:lower()
					if nameLower:find("double") or nameLower:find("doubledoor") or nameLower:find("double_door") or nameLower:find("doubleswingdoor") then
	...
	CombatTab:CreateDropdown({
		...
		Callback = function(Option)
			aimbotTargetPart = (Option == "Torso / Root") and "HumanoidRootPart" or "Head"
		end,
	})
	...
	CombatTab:CreateKeybind({
		...
		Callback = function(KeyHeld) aimbotKeyHold = KeyHeld end,
	})
	...
	CombatTab:CreateToggle({
		...
		Callback = function(Value) fastReloadEnabled = Value end,
	})
	...
	RobberyTab:CreateToggle({
		...
		Callback = function(Value)
			...
			for _, hl in ipairs(atmHighlights) do if hl and hl.Parent then hl:Destroy() end end
			atmHighlights = {}
	...
	RobberyTab:CreateColorPicker({
		...
		Callback = function(Value)
			atmColor = Value
			for _, hl in ipairs(atmHighlights) do if hl and hl.Parent then hl.FillColor = atmColor end end
		end
	})
	...
	RobberyTab:CreateToggle({
		...
		Callback = function(Value)
			...
			for _, hl in ipairs(airdropHighlights) do if hl and hl.Parent then hl:Destroy() end end
			airdropHighlights = {}
	...
	RobberyTab:CreateColorPicker({
		...
		Callback = function(Value)
			airdropColor = Value
			for _, hl in ipairs(airdropHighlights) do if hl and hl.Parent then hl.FillColor = airdropColor end end
		end
	})
	...
	OtherTab:CreateToggle({
		...
		Callback = function(Value)
			...
			for _, hl in ipairs(doorHighlights) do if hl and hl.Parent then hl:Destroy() end end
			doorHighlights = {}
	...
	OtherTab:CreateToggle({
		...
		Callback = function(Value)
			...
			for _, hl in ipairs(slideDoorHighlights) do if hl and hl.Parent then hl:Destroy() end end
			slideDoorHighlights = {}
	...
	-- ИСПРАВЛЕННЫЙ ТУМБЛЕР DOUBLE DOORS
	OtherTab:CreateToggle({
		...
		Callback = function(Value)
			...
			for _, hl in ipairs(doubleDoorHighlights) do if hl and hl.Parent then hl:Destroy() end end
	...
	OtherTab:CreateColorPicker({
		...
		Callback = function(Value)
			doorColor = Value
			for _, hl in ipairs(doorHighlights) do if hl and hl.Parent then hl.FillColor = doorColor end end
		end
	})
	...
	OtherTab:CreateColorPicker({
		...
		Callback = function(Value)
			...
			for _, hl in ipairs(slideDoorHighlights) do if hl and hl.Parent then hl.FillColor = slideDoorColor end end
		end
	})
	...
	OtherTab:CreateColorPicker({
		...
		Callback = function(Value)
			...
			for _, hl in ipairs(doubleDoorHighlights) do if hl and hl.Parent then hl.FillColor = doubleDoorColor end end
		end
	})
